...
The `investigate` method will write each model to be tested to a new sub-directory of "`r path_to_write_to_1L_chr`" called "A_Candidate_Mdls_Cmprsn". Also written to that sub-directory are a number of plots for each model. 

```{r, fig.cap=paste0("Plot"1:5), echo=FALSE, message=F}
depict(x,
       mdl_idcs_int = 2)

```

```{r, fig.cap=paste0("Plot"1), echo=FALSE, message=F}
depict(x,
       mdl_idcs_int = 1,
       plt_indcs_int = 1)
```
