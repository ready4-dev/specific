...
```{r}
params_ls <- list(results_ls = list(tables_ls = list(tenf_sngl_predr_tb = procure(x,
        what_1L_chr = "mdl_cmprsn"))),
        output_type_1L_chr = "HTML")
#params_ls$results_ls$tables_ls$tenf_sngl_predr_tb
```
...
print_ten_folds_tbl(params_ls,
  ...
  kableExtra::add_header_above(NULL)#c(" " = 1, "Training model fit (averaged over 10 folds)" = 3, "Testing model fit (averaged over 10 folds)" = 3)
```

The `investigate` method will write each model to be tested to a new sub-directory of "`r path_to_write_to_1L_chr`" called "A_Candidate_Mdls_Cmprsn". Also written to that sub-directory are a number of plots for each model. The `investigate` method also outputs a table summarising the performance of each of the candidate models.
...
exhibit_SpecificProject(x,
        ...
        what_1L_chr = "mdl_cmprsn",
        type_1L_chr = "results",
        heading_grps_chr = c(" " = 1, "Training model fit (averaged over 10 folds)" = 3, "Testing model fit (averaged over 10 folds)" = 3))
```
