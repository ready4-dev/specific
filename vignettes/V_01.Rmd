...
#                         depnt_var_min_max_dbl = c(-0.9999,0.9999))
```
...
```
We can choose to select just a subset of these to explore using the `renewSlot` method. As this is an illustrative example, we have restricted the models we will explore to just three types, passing the relevant row numbers to the `slice_idxs_int` argument.

```{r warning=F}
X <- renewSlot(X,
               "b_SpecificParameters@candidate_mdls_lup",
               slice_idxs_int = c(1L,5L,6L))
```
...
Before we start to use the data stored in `X` to undertake modelling, we must first validate that it contains all necessary (and internally consistent) data by using the `ratify` method.
