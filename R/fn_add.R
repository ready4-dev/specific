    ...
    sd_dbl = NA_real_, utl_cls_fn = NULL, utl_min_val_1L_dbl = -1) 
{
    ...
    predictions_dbl <- predict_vals(data_tb = data_tb, tfmn_1L_chr = tfmn_1L_chr, 
        model_mdl = model_mdl, min_max_vals_dbl = min_max_vals_dbl, 
        impute_1L_lgl = impute_1L_lgl, new_data_is_1L_chr = new_data_is_1L_chr, 
        var_cls_fn = utl_cls_fn, is_brms_mdl_1L_lgl = is_brms_mdl_1L_lgl, 
        force_new_data_1L_lgl = force_new_data_1L_lgl, predn_type_1L_chr = predn_type_1L_chr, 
        sd_dbl = sd_dbl)
    data_tb <- data_tb %>% dplyr::mutate(`:=`(!!rlang::sym(depnt_var_nm_1L_chr), 
        predictions_dbl))
    if (!is.null(predr_vars_nms_chr)) {
        derived_predrs_chr <- purrr::map(predr_vars_nms_chr, 
            ~paste0(.x, c("_baseline", "_change", "_scaled", 
                "_unscaled"))) %>% purrr::flatten_chr() %>% intersect(names(data_tb))
