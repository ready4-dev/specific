...
To start with, we create a new instance of a `SpecificInitiator` object containing data we have just ingested.

```{r}
x <- SpecificInitiator(a_YouthvarsProfile = x) 
...
Finally, we add `y` (model parameters) to `x` (the `SpecificInitiator`).
